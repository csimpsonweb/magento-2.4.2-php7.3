...
sub vcl_recv {
    ...
    if (req.url ~ "/checkout" || req.url ~ "/catalogsearch" || req.url ~ "/rest" || req.url ~ "/wp") {
        return (pass);
    }
...
#    set req.url = regsub(req.url, "^http[s]?://", "");
    ...
    if (req.url ~ "^/(pub/)?(media|static)/.*\.(ico|css|js|jpg|jpeg|png|gif|tiff|bmp|mp3|ogg|svg|swf|woff|woff2|eot|ttf|otf|webp)$") {
...
sub vcl_backend_response {
    ...
    # Test webp
    if (beresp.http.content-type ~ "image/webp") {
...
sub vcl_hit {
    ...
    if (std.healthy(req.backend_hint)) {
        if (std.random(0, 100) < 5) {
            set req.http.PS-ShouldBeacon = "P1xLn05kR7vs13eP7e81G1N1b810D47e97C";
            return (pass);
        }
        if (obj.ttl + 300s > 0s) {
